...
def devision_predict(number: int = 1) -> int:  
    ...
    Автор кода: Алексей Кияница, Петрозаводск, Россия
    
    Используется метод ловли льва в прериях: слушаем, с какой стороны рычит, и делим прерию пополам,
    после чего идем в рычащую половину и делим ее пополам, и в какой-то момент встретимся со львом на радость льву
    ...
    while True:
        ...
        if (predict_number == number) and (min_n <= predict_number <= max_n):
        ...
        elif (predict_number != number) and ((max_n - min_n) == 1) and (min_n <= predict_number <= max_n):
            ...
            if (predict_number < number):
                min_n = int(predict_number)
                continue
            elif (predict_number > number):
                max_n = int(predict_number)
                continue
